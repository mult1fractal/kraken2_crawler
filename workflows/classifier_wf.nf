...
workflow classifier_wf {
    take:   fastq
    main:   
             if (params.custom_db) { custom_db_ch = Channel
                                            .fromPath( params.custom_db, checkIfExists: true)
                                            .view()}

            if (params.custom_db) { kraken2(fastq, custom_db_ch) }
            else { kraken2(fastq, download_database_kraken2()) }
